...
import { FootballActivityFormValues } from '../../../../App/models/footballActivity';
...
export default observer(function ActivityForm(){
    ...
    const {footballActivityStore} = useStore();
    const { createFootballActivity, updateFootballActivity, loadFootballActivity, loadingInitial} = footballActivityStore
    const {id} = useParams<{id: string}>();

    const [activity, setActivity] = useState<FootballActivityFormValues>(new FootballActivityFormValues());
    ...
    useEffect(() => {
        if (id) {
            loadFootballActivity(id).then(loadedActivity => {
                const activityFormValues = new FootballActivityFormValues(loadedActivity);
    ...
    }, [id, loadFootballActivity]);



    function handleFormSubmit(activity: FootballActivityFormValues) {
        if (!activity.id) {
            let newActivity = {
                ...activity,
                id: uuid()
            };
            createFootballActivity(newActivity).then(() => history.push(`/activities/${newActivity.id}`))
        } else {
            updateFootballActivity(activity).then(() => history.push(`/activities/${activity.id}`))
        }
    }
